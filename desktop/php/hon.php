...
foreach ($eqLogics as $eqLogic) {
    ...
    if (is_object($connectionCmd) && $connectionCmd->execCmd() == 'connected') {
        $connectedEquipments++;
    }
}
...
<div class="row row-overflow">
    ...
    <div class="col-xs-12 eqLogicThumbnailDisplay">
        ...
        <div class="eqLogicThumbnailContainer">
            ...
            <div class="cursor bt_syncHonEquipments logoSecondary" title="{{Synchroniser les équipements hOn}}">
                <i class="fas fa-sync"></i>
                <br>
                <span>{{Synchroniser}}</span>
            </div>
            <div class="cursor bt_regenerateJsons logoSecondary" title="{{Régénérer les fichiers JSON}}">
                <i class="fas fa-file-code"></i>
                <br>
                <span>{{Régénérer JSONs}}</span>
            </div>
            <div class="cursor bt_refreshTokens logoSecondary" title="{{Actualiser les tokens d'authentification}}">
        ...
        <?php if ($totalEquipments == 0): ?>
            <div class="alert alert-info text-center">
                ...
                <br><br>
                <p>{{Pour commencer, cliquez sur}} <strong>{{Synchroniser}}</strong> {{pour détecter automatiquement vos équipements hOn.}}</p>
                <p><small>{{Assurez-vous d'avoir configuré vos identifiants dans la configuration du plugin.}}</small></p>
            </div>
        <?php else: ?>
            <!-- Statistiques -->
            <div class="alert alert-info">
                <div class="row">
                    ...
                    <div class="col-sm-3 text-center">
                        <h4 class="margin-0 <?php echo $connectedEquipments > 0 ? 'text-success' : 'text-warning'; ?>"><?php echo $connectedEquipments; ?></h4>
                        <small>{{Connectés}}</small>
                    </div>
                    <div class="col-sm-3 text-center">
                        <h4 class="margin-0 text-info"><?php echo $programsCount; ?></h4>
        ...
        <div class="eqLogicThumbnailContainer">
            ...
            foreach ($eqLogics as $eqLogic) {
                ...
                $macFilename = $eqLogic->getConfiguration('mac_filename', 'N/A');
    ...
    <div class="col-xs-12 eqLogic" style="display: none;">
        ...
        <ul class="nav nav-tabs" role="tablist">
            ...
            <li role="presentation">
                <a href="#programtab" aria-controls="programs" role="tab" data-toggle="tab">
                    <i class="fas fa-play-circle"></i> {{Programmes}}
                </a>
            </li>
            <li role="presentation">
                <a href="#statustab" aria-controls="status" role="tab" data-toggle="tab">
        ...
        <div class="tab-content">
                    ...
                    <fieldset>
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="eqLogicAttr form-control" data-l1key="configuration" data-l2key="applianceType" placeholder="{{Type d'appareil}}" readonly>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="eqLogicAttr form-control" data-l1key="configuration" data-l2key="mac_filename" placeholder="{{Nom de fichier MAC}}" readonly>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-4 control-label">{{Nombre de programmes}}</label>
                                <div class="col-sm-8">
                                    <input type="text" class="eqLogicAttr form-control" data-l1key="configuration" data-l2key="programsCount" placeholder="{{Nombre de programmes}}" readonly>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-4 control-label">{{Nombre de paramètres}}</label>
                                <div class="col-sm-8">
                                    <input type="text" class="eqLogicAttr form-control" data-l1key="configuration" data-l2key="settingsCount" placeholder="{{Nombre de paramètres}}" readonly>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-4 control-label">{{Dernière synchronisation}}</label>
                                <div class="col-sm-8">
                                    <span class="form-control-static eqLogicAttr" data-l1key="configuration" data-l2key="lastSync">{{Jamais}}</span>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-4 control-label">{{Actions}}</label>
                                <div class="col-sm-8">
                                    <a class="btn btn-primary btn-sm bt_refreshDevice" title="{{Rafraîchir les données de cet équipement}}">
                                        <i class="fas fa-sync"></i> {{Rafraîchir}}
                                    </a>
                                    <a class="btn btn-info btn-sm bt_showJsonFiles" title="{{Voir les fichiers JSON associés}}">
                                        <i class="fas fa-file-code"></i> {{Voir JSONs}}
                                    </a>
                                </div>
                            </div>
                        </div>
            ...
            <div role="tabpanel" class="tab-pane" id="programtab">
                <div id="programsContainer">
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle"></i> 
                        {{Les programmes disponibles seront chargés automatiquement depuis les fichiers JSON de l'équipement.}}
                    </div>
            ...
            <div role="tabpanel" class="tab-pane" id="statustab">
                <div id="statusContainer">
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle"></i> 
                        {{Les informations de statut seront chargées automatiquement.}}
                    </div>
...
<!-- Modal pour l'affichage des fichiers JSON -->
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><i class="fas fa-file-code"></i> {{Fichiers JSON de l'équipement}}</h4>
            </div>
...
<!-- Modal pour le lancement de programmes -->
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><i class="fas fa-play-circle"></i> {{Lancer un programme}}</h4>
            </div>
            <div class="modal-body">
                <form id="programForm">
                    <div class="form-group">
                    ...
                    <div id="programParameters"></div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">{{Annuler}}</button>
                <button type="button" class="btn btn-primary" id="launchProgram">{{Lancer}}</button>
            </div>
        </div>
    </div>
</div>
