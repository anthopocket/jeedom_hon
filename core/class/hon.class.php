...
class hon extends eqLogic {
    ...
    public function postSave() {
        // Créer les commandes SEULEMENT si l'équipement vient d'être créé
        // (pas de commandes existantes)
        $existingCommands = $this->getCmd();
        
        if (empty($existingCommands)) {
            log::add('hon', 'info', 'Création des commandes pour le nouvel équipement : ' . $this->getName());
            $this->createCommands();
        }
    }
