    ...
    private static function updateDeviceFromQuickData($deviceResult) {
        try {
            ...
            $keyMapping = [
                ...
                'pause_status' => 'pause_info', // ← CHANGÉ POUR ÉVITER CONFLIT
...
';
    ...
    /**
     * Crée uniquement les commandes info qui sont dans le mapping
     */
    private function createMappedInfoCommands() {
        // Liste des commandes info à créer (basée sur votre mapping)
        $infoCommands = [
            ...
            'pause_info' => ['name' => 'État pause', 'subtype' => 'binary'], // ← CHANGÉ
        ...
        log::add('hon', 'info', count($infoCommands) . ' commandes info créées pour ' . $this->getName());
    }

    /**
     * Crée uniquement les commandes d'action essentielles
     */
    private function createEssentialActionCommands() {
        // Commandes d'action essentielles
        $this->createActionCommand('pause_action', 'Pause', 'pause');
        $this->createActionCommand('resume', 'Reprendre', 'resume');
        $this->createActionCommand('stop', 'Arrêter', 'stop');
        $this->createActionCommand('refresh', 'Rafraîchir', 'refresh');
        
        log::add('hon', 'info', '4 commandes action essentielles créées pour ' . $this->getName());
    }
