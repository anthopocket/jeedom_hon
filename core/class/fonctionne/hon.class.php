...
class hon extends eqLogic {
    ...
    public static function refreshAllDevices() {
        ...
        foreach ($eqLogics as $eqLogic) {
            if ($eqLogic->getIsEnable()) {
                ...
                $lastRefresh = $eqLogic->getConfiguration('lastRefresh', 0);
                $currentTime = time();
                
                if (($currentTime - $lastRefresh) >= 60) { // 60 secondes = 1 minute
                    log::add('hon', 'debug', 'Rafraîchissement programmé pour : ' . $eqLogic->getName());
                    $eqLogic->refreshInfo();
                } else {
                    log::add('hon', 'debug', 'Rafraîchissement ignoré pour : ' . $eqLogic->getName() . ' (dernière mise à jour il y a ' . ($currentTime - $lastRefresh) . ' secondes)');
                }
            }
        }
    }
    ...
    public static function getCachedTokens() {
        ...
        if (($currentTime - $lastTokenTime) < 19800) {
            ...
            if (!empty($cachedIdToken) && !empty($cachedCognitoToken)) {
                log::add('hon', 'debug', 'Utilisation des tokens en cache');
                return [
        ...
        log::add('hon', 'info', 'Tokens expirés ou inexistants, régénération...');
    ...
    public function refreshInfo() {
        ...
        if (empty($macAddress)) {
            log::add('hon', 'warning', 'Pas d\'adresse MAC pour l\'équipement : ' . $this->getName());
            return;
        }
        
        // Vérifier la dernière mise à jour pour éviter les appels trop fréquents
        $lastRefresh = $this->getConfiguration('lastRefresh', 0);
        $currentTime = time();
        
        if (($currentTime - $lastRefresh) < 60) { // Minimum 1 minute entre les rafraîchissements
            log::add('hon', 'debug', 'Rafraîchissement ignoré pour ' . $this->getName() . ' (dernière mise à jour il y a ' . ($currentTime - $lastRefresh) . ' secondes)');
            return;
        }
        ...
        try {
            ...
            if (!$tokens) {
                log::add('hon', 'error', 'Impossible de récupérer les tokens pour rafraîchir : ' . $this->getName());
                return;
            }
            ...
            if ($statusData && isset($statusData['data']) && is_array($statusData['data'])) {
                foreach ($statusData['data'] as $sensor => $value) {
                    ...
                    if (is_object($cmd)) {
                        ...
                        $cmd->event($value);
                        log::add('hon', 'debug', 'Mise à jour ' . $sensor . ' = ' . $value . ' pour : ' . $this->getName());
                    }
                }
                ...
                log::add('hon', 'debug', 'Informations rafraîchies pour : ' . $this->getName());
            } else {
                log::add('hon', 'warning', 'Aucune donnée reçue pour : ' . $this->getName());
            }
    ...
    private function getDeviceData($idToken, $cognitoToken, $macAddress) {
        ...
        $cmd = 'cd ' . __DIR__ . '/../../resources && python3 hon_extraction.py --mac ' . 
               ...
               ' --output-format jeedom';
        
        log::add('hon', 'debug', 'Exécution commande statut : ' . $cmd);
        
        $output = shell_exec($cmd . ' 2>&1');
        log::add('hon', 'debug', 'Sortie statut : ' . $output);
