...
class ConfigManager:
    """Gestionnaire de configuration pour les paramètres dynamiques"""
    
    @staticmethod
    def load_from_args():
        """Charge la configuration depuis les arguments de ligne de commande"""
        parser = argparse.ArgumentParser(description='HON Device Data Extractor')
        parser.add_argument('--mac', required=True, help='MAC address of the device')
        parser.add_argument('--type', required=True, choices=['WM', 'TD', 'WD'], help='Appliance type')
        parser.add_argument('--id-token', required=True, help='ID Token')
        parser.add_argument('--cognito-token', required=True, help='Cognito Token')
        parser.add_argument('--config-file', help='Path to JSON config file')
        parser.add_argument('--output-format', choices=['json', 'jeedom'], default='json', help='Output format')
    ...
    def load_from_file(config_path):
        ...
        required_fields = ['mac_address', 'appliance_type', 'id_token', 'cognito_token']
    ...
    def load_from_env():
        ...
        config = {
            'mac_address': os.getenv('HON_MAC_ADDRESS'),
            'appliance_type': os.getenv('HON_APPLIANCE_TYPE'),
            'id_token': os.getenv('HON_ID_TOKEN'),
            'cognito_token': os.getenv('HON_COGNITO_TOKEN')
        }
...
class HonSensorExtractor:
    ...
    def _calculate_program_name(self, config):
        """Calcule le nom du programme"""
        program_code = self.device.get("prCode", "")
        
        # Mapping basique des codes programmes
        program_names = {
            "113": "Cotton 30°C",
            "114": "Cotton 40°C",
            "115": "Cotton 60°C",
...
async def main():
    """Fonction principale avec paramètres dynamiques"""
    
    # Chargement de la configuration selon la méthode disponible
    config = None
    
    try:
        # Méthode 1: Arguments de ligne de commande
        if len(sys.argv) > 1:
            args = ConfigManager.load_from_args()
            config = {
            ...
            # Si un fichier de config est spécifié, on l'utilise en priorité
            if args.config_file:
                file_config = ConfigManager.load_from_file(args.config_file)
                config.update(file_config)
        
        # Méthode 2: Fichier de configuration (si pas d'arguments)
        elif os.path.exists('hon_config.json'):
            config = ConfigManager.load_from_file('hon_config.json')
            config['output_format'] = config.get('output_format', 'json')
        
        # Méthode 3: Variables d'environnement
        else:
            config = ConfigManager.load_from_env()
            config['output_format'] = os.getenv('HON_OUTPUT_FORMAT', 'json')
            
    except Exception as e:
        print(f"❌ Erreur de configuration: {str(e)}")
        print("\n📋 Méthodes de configuration disponibles:")
        print("1. Arguments: --mac MAC --type WM --id-token TOKEN --cognito-token TOKEN")
        print("2. Fichier: hon_config.json")
        print("3. Variables d'environnement: HON_MAC_ADDRESS, HON_APPLIANCE_TYPE, etc.")
        return {}
    
    mac_address = config['mac_address']
    appliance_type = config['appliance_type']
    id_token = config['id_token']
    cognito_token = config['cognito_token']
    output_format = config.get('output_format', 'json')
    
    print(f"🔄 Configuration chargée pour {appliance_type} ({mac_address})")
    
    hon = HonConnection(id_token, cognito_token)
    ...
    try:
        ...
        appliance_type_enum = APPLIANCE_MAPPING.get(appliance_type, ApplianceType.WASHING_MACHINE)
        
        print(f"📦 Extraction des sensors pour {appliance_type_enum.name}...")
        ...
        extractor = HonSensorExtractor(device, appliance_type_enum, parameters, context)
        sensors = extractor.extract_sensors()

        # Structure finale du JSON
        result = {
            "device_info": {
                "name": f"hOn {appliance_type_enum.name.replace('_', ' ').title()}",
                "mac_address": mac_address,
                "appliance_type": appliance_type,
                "appliance_name": appliance_type_enum.name.replace('_', ' ').title(),
                "last_update": datetime.now(timezone.utc).isoformat()
            },
            "sensors": sensors,
            "metadata": {
                ...
                "appliance_type": appliance_type_enum.name,
                "cognito_token": hon._cognitoToken  # Token mis à jour
            }
        }

        print(f"\n✅ Extraction terminée avec succès !")
        print(f"📊 {len(sensors)} sensors extraits pour {appliance_type_enum.name.replace('_', ' ')}")
        
        if output_format == 'jeedom':
            # Format spécial pour Jeedom (simplifié)
            jeedom_result = {
                "success": True,
                "data": {sensor_id: sensor_data["value"] for sensor_id, sensor_data in sensors.items()},
                "device_info": result["device_info"],
                "updated_tokens": {
                    "cognito_token": hon._cognitoToken
                }
            }
            print(json.dumps(jeedom_result, indent=2, ensure_ascii=False))
            return jeedom_result
        else:
