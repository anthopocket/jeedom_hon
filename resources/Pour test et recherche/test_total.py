...
def extract_binary_sensors(parameters, context):
    """Extrait les capteurs binaires (code 1)"""
    binary_sensors = {}
    
    # Liste des attributs binaires à afficher
    binary_attributes = [
        "onOffStatus", "doorStatus", "doorStatusZ1", "doorStatusZ2", "lightStatus",
        "doorLockStatus", "lockStatus", "remoteCtrValid", "preheatStatus",
        "healthMode", "saltStatus", "rinseAidStatus", "defrostStatus", "pause"
    ]
    ...
    for key in binary_attributes:
        if key in parameters:
            ...
            # Détermination des états selon le type de capteur
            if key in ["doorLockStatus", "lockStatus"]:
                state_on = "Verrouillé"
                state_off = "Déverrouillé"
            elif key == "pause":
                state_on = "En pause"
                state_off = "Non en pause"
            else:
                state_on = "Activé/Ouvert"
                state_off = "Désactivé/Fermé"
            ...
            binary_sensors[key] = {
                ...
                "state_on": state_on,
                "state_off": state_off,
                "last_update": parameters[key]["lastUpdate"],
                "source": "direct"
            }
    
    # Gestion spéciale de onOffStatus via lastConnEvent si non présent
    if "onOffStatus" not in parameters and "lastConnEvent" in context:
        status = context["lastConnEvent"]["category"] == "CONNECTED"
        binary_sensors["onOffStatus"] = {
            "value": status,
            "name": "onOffStatus",
            "state_on": "Connecté",
            "state_off": "Déconnecté",
            "last_update": context["lastConnEvent"]["instantTime"],
            "source": "lastConnEvent"
        }
    
    return binary_sensors

def extract_numeric_parameters(parameters):
    """Extrait tous les paramètres numériques (code 2)"""
    numeric_params = {}
...
async def main():
    ...
    try:
        ...
        print("📦 Extraction des données complètes...")
        
        # 1. Extraction des capteurs binaires (code 1)
        binary_sensors = extract_binary_sensors(parameters, context)
        
        # 2. Extraction des paramètres numériques bruts (code 2)
        numeric_parameters = extract_numeric_parameters(parameters)
        
        # 3. Extraction des sensors formatés (code 3)
        device_data = {}
        for key, param in parameters.items():
            if isinstance(param, dict) and "parNewVal" in param:
        ...
        formatted_sensors = washing_data.extract_all_sensors()
        ...
        complete_data = {
            ...
            "metadata": {
                ...
                "total_numeric_parameters": len(numeric_parameters),
                "total_formatted_sensors": len(formatted_sensors)
            }
        }

        print("\n✅ Données complètes extraites avec succès !")
        print(f"📊 {len(binary_sensors)} capteurs binaires")
        print(f"🔢 {len(numeric_parameters)} paramètres numériques")  
        print(f"🎯 {len(formatted_sensors)} sensors formatés\n")
        
        print(json.dumps(complete_data, indent=4, ensure_ascii=False))
